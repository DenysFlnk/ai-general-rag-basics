...
class MicrowaveRAG:
    ...
    def _create_new_index(self) -> VectorStore:
        ...
        vector_store = FAISS.from_documents(splitted_docs, self.embeddings)
    ...
    def retrieve_context(self, query: str, k: int = 4, score=0.3) -> str:
        ...
        for doc, score in documents:
            print(f"- {doc.page_content}, similarity score: {score}")
            context_parts.append(doc.page_content)

        print("=" * 100)
        return "\n\n".join(
            context_parts
        )  # will join all chunks ion one string with `\n\n` separator between chunks

    def augment_prompt(self, query: str, context: str) -> str:
        print(f"\n🔗 STEP 2: AUGMENTATION\n{'-' * 100}")

        augmented_prompt = USER_PROMPT.format(query=query, context=context)
...
main(
    MicrowaveRAG(
        ...
        ChatOpenAI(temperature=0.0, model="gpt-4o", api_key=SecretStr(OPENAI_API_KEY)),
    )
)
